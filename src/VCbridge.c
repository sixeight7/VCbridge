...
#define VCONTROLLER_MIDI_CHANNEL 15   // (0 - 15)
...
void arg_set_defaults(arguments_t *arguments)
{
	...
	char *name_tmp		= (char *)"VCbridge";
	strncpy(arguments->serialdevice, serialdevice_temp, MAX_DEV_STR_LEN);
	strncpy(arguments->name, name_tmp, MAX_DEV_STR_LEN);
}

const char *argp_program_version     = "VCbridge 1.0";
const char *argp_program_bug_address = "sixeight/vguitarforums.com";
static char doc[]       = "VCbridge - Connect serial port devices to ALSA MIDI programs!";
...
{
	// Create ports for the VController bridge
	char portname[64];
	int p;
	...
	for (p = 0; p < NUMBER_OF_PORTS; p++) {
		sprintf(portname, "PORT %d", p);
		if ((port_id[p] = snd_seq_create_simple_port(*seq, portname,
			SND_SEQ_PORT_CAP_READ|SND_SEQ_PORT_CAP_SUBS_READ|
			SND_SEQ_PORT_CAP_WRITE|SND_SEQ_PORT_CAP_SUBS_WRITE,
			SND_SEQ_PORT_TYPE_MIDI_GENERIC|SND_SEQ_PORT_TYPE_APPLICATION)) < 0)
		{
			fprintf(stderr, "Error creating sequencer ports.\n");
			exit(1);
		}
	}
		...
		switch (ev->type)
		{
			...
			case SND_SEQ_EVENT_NOTEOFF:
				...
				if (!arguments.silent && arguments.verbose)
					printf("Alsa %u  0x%02X Note off           %03u %03u %03u\n", inport, bytes[0]&0xF0, bytes[0]&0xF, bytes[1], bytes[2]);
				break;
			...
			case SND_SEQ_EVENT_NOTEON:
				...
				bytes[2] = ev->data.note.velocity;
				if (!arguments.silent && arguments.verbose)
					printf("Alsa %u  0x%02X Note on            %03u %03u %03u\n", inport, bytes[0]&0xF0, bytes[0]&0xF, bytes[1], bytes[2]);
				break;
			...
			case SND_SEQ_EVENT_KEYPRESS:
				...
				bytes[2] = ev->data.note.velocity;
				if (!arguments.silent && arguments.verbose)
					printf("Alsa %u  0x%02X Pressure change    %03u %03u %03u\n", inport, bytes[0]&0xF0, bytes[0]&0xF, bytes[1], bytes[2]);
				break;
			...
			case SND_SEQ_EVENT_CONTROLLER:
				...
				if (!arguments.silent && arguments.verbose)
					printf("Alsa %u  0x%02X Controller change  %03u %03u %03u\n", inport, bytes[0]&0xF0, bytes[0]&0xF, bytes[1], bytes[2]);
				break;
			...
			case SND_SEQ_EVENT_PGMCHANGE:
				...
				if (!arguments.silent && arguments.verbose)
					printf("Alsa %u  0x%02X Program change     %03u %03u\n", inport, bytes[0]&0xF0, bytes[0]&0xF, bytes[1]);
				break;
			...
			case SND_SEQ_EVENT_CHANPRESS:
				...
				bytes[1] = ev->data.control.value;
				if (!arguments.silent && arguments.verbose)
					printf("Alsa %u  0x%02X Channel change     %03u %03u %03u\n", inport, bytes[0]&0xF0, bytes[0]&0xF, bytes[1], bytes[2]);
				break;
			...
			case SND_SEQ_EVENT_PITCHBEND:
				...
				if (!arguments.silent && arguments.verbose)
					printf("Alsa %u  0x%02X Pitch bend         %03u %5d\n", inport, bytes[0]&0xF0, bytes[0]&0xF, ev->data.control.value);
				break;
				
            case SND_SEQ_EVENT_SYSEX:
                sysex_len = ev->data.ext.len;
                if (!arguments.silent && arguments.verbose)
					printf("Alsa %u  0xF0 System Exclusive   ", inport);
                int i;
...
void write_midi_to_alsa(snd_seq_t* seq, int port_out_id, char *buf, int buflen)
{
   ...
   */
	...
	switch (operation)
	{
		case 0x80:
			if (!arguments.silent && arguments.verbose)
				printf("Serial%u 0x%02X Note off           %03u %03u %03u\n", outport, operation, channel, param1, param2);
			snd_seq_ev_set_noteoff(&ev, channel, param1, param2);
			break;

		case 0x90:
			if (!arguments.silent && arguments.verbose)
				printf("Serial%u 0x%02X Note on            %03u %03u %03u\n", outport, operation, channel, param1, param2);
			snd_seq_ev_set_noteon(&ev, channel, param1, param2);
			break;

		case 0xA0:
			if (!arguments.silent && arguments.verbose)
				printf("Serial%u 0x%02X Pressure change    %03u %03u %03u\n", outport, operation, channel, param1, param2);
			snd_seq_ev_set_keypress(&ev, channel, param1, param2);
			break;

		case 0xB0:
			if (!arguments.silent && arguments.verbose)
				printf("Serial%u 0x%02X Controller change  %03u %03u %03u\n", outport, operation, channel, param1, param2);
			snd_seq_ev_set_controller(&ev, channel, param1, param2);
			break;

		case 0xC0:
			if (!arguments.silent && arguments.verbose)
				printf("Serial%u 0x%02X Program change     %03u %03u\n", outport, operation, channel, param1);
			snd_seq_ev_set_pgmchange(&ev, channel, param1);
			break;

		case 0xD0:
			if (!arguments.silent && arguments.verbose)
				printf("Serial%u 0x%02X Channel change     %03u %03u\n", outport, operation, channel, param1);
		...
		case 0xE0:
			...
			if (!arguments.silent && arguments.verbose)
				printf("Serial%u 0x%02X Pitch bend         %03u %05i\n", outport, operation, channel, param1);
		...
		case 0xF0:
            if (buf[0] == 0xF0) {
				if (!arguments.silent && arguments.verbose) {
					printf("Serial%u 0x%02X System Exclusive   ", outport, operation);
				    int i;
					for (i=0; i < buflen; i++) {
						printf("%02X ", buf[i]);
					}
...
void* read_midi_from_serial_port(void* seq)
{
	...
	while (run)
	{
		...
		bytesleft = BUF_SIZE - 1;   //reset bytesleft to maximum value
